...
function updateContentVisibility(lang) {
    ...
    // إظهار المحتوى العربي وإخفاء الإنجليزي
    if (lang === 'ar') {
        langAr.forEach(el => el.style.display = 'inline');
        langEn.forEach(el => el.style.display = 'none');
        document.getElementById('language-toggle').innerText = 'English / عربي';
    } 
    // إظهار المحتوى الإنجليزي وإخفاء العربي
    else {
        langAr.forEach(el => el.style.display = 'none');
        langEn.forEach(el => el.style.display = 'inline');
        document.getElementById('language-toggle').innerText = 'عربي / English';
    }
}

// الدالة التي تعمل عند تحميل الصفحة للتحقق من اللغة المحفوظة
// (مهمة: يتم استدعاؤها في وسم <body> في index.html)
